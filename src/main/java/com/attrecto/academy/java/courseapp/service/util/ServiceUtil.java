...
public class ServiceUtil {
	...
	public Course findCourseById(int id) {
		...
		if(course.isEmpty()) {
			throw new RuntimeException(String.format("Course cannot be found with id: %s", id));
		}
		return course.get();
	}
	...
	public User findUserById(Integer id) {
		...
		if(user.isEmpty()) {
			throw new RuntimeException(String.format("User cannot be found with id: %s", id));
		}
		return user.get();
	}
